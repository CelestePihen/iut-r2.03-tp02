...
class PionNoir extends Pion {
	...
	public ArrayList<Position> getDeplacementPossible(Plateau pl) throws ErreurCoordonneesException {
		...
		if (this.getPosition().getY() != 0) { // Si le pion n'est pas arrivé au bout - Est ce vraiment éncessaire ? Car
												// les pions se transforment en autre chose au bout du plateau.
			if (pl.getCase(this.getPosition().getX(), this.getPosition().getY() - 1) == null) // s'il n'y a rien devant
																								// lui, il peut avancer.
				liste.add(new Position(this.getPosition().getX(), this.getPosition().getY() - 1));

			// Test s'il peut avancer de deux cases
			if (this.getPosition().getY() == 6 && liste.size() == 1) { // le pion n'a pas encore bougé et qu'il n'y a
																		// pas de pièce devant lui donc que le test
																		// précédent est passé
